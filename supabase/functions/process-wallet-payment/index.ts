...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: walletTransaction, error: transactionError } = await supabase.from('wallet_transactions').select('*').eq('merchant_order_id', merchantOrderId).eq('transaction_type', 'deposit').single();
    if (transactionError) {
    ...
    if (verificationData.verified && verificationData.status === 'SUCCESS') {
      ...
      let { data: wallet, error: walletError } = await supabase.from('wallets').select('*').eq('user_id', walletTransaction.user_id).single();
      ...
      const { error: statusUpdateError } = await supabase.from('wallet_transactions').update({
        status: 'completed',
